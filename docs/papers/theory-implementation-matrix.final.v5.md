...
> **버전**: v5 — 투고용 최종본 (v4의 Codex·critic 통합 반영 + document-specialist 인용 메타데이터 검증 patch 3건 + 이미지 syntax 4건 + 학습 잔디 anonymized exemplar 적용. verifier APPROVE 판정 상태)
...
이 연구는 한 대학원 교육공학 학회가 운영하는 디지털 인프라(yonsei-edtech)에 교육공학의 10개 핵심 이론이 어떻게 구현되어 있는지를 단일 사례 분석한다. 단일 이론을 단일 시스템에 적용한 기존 연구와 달리, 사례는 자기조절학습, 자기결정성 이론, 인지부하 이론, 다중매체 학습 이론, 실천공동체, 인지 도제, 분산 인지, 형성평가, 자기효능감·게이미피케이션, 개방 과학·절차적 정의의 열 가지 이론이 9개 도메인 위에서 동시에 구현되는 통합 디자인을 보여 준다. 분석은 사이트 소스 코드(약 5만 라인, 약 290개 컴포넌트)와 30건의 설계 문서를 1차 자료로 삼고, 7개월간의 익명 활동 로그를 보조 참조 자료원으로 검토하였다(운영 효과의 실증은 본 논문 범위 바깥의 후속 트랙으로 이전). 4단계 매핑 강도 기준(핵심·중요·부분·잠재)으로 90셀(10×9) 평가가 이루어졌다. 기여는 세 갈래이다. 매트릭스를 학술 가독성 높은 시각화로 제시한다. 매핑 강도를 행·열 평균으로 정량화하여 이론 통합도와 도메인 이론 밀도를 분석한다. 이론 간 시너지·긴장·보완 패턴을 6개 사례로 식별한다. 분석 결과 자기효능감·게이미피케이션과 자기결정성 이론이 가장 폭넓게 분포하였고, 출판 마법사 도메인은 인지부하 이론 중심으로 응집되었다. 후속 실증 연구의 우선순위 결정에 활용 가능한 6개 트랙이 도출되었다.
...
This study analyzes how ten core educational technology theories are simultaneously implemented in a digital infrastructure (yonsei-edtech) operated by a graduate-level educational technology society. Unlike prior studies applying a single theory to a single system, the case demonstrates the integrated implementation of Self-Regulated Learning, Self-Determination Theory, Cognitive Load Theory, Cognitive Theory of Multimedia Learning, Communities of Practice, Cognitive Apprenticeship, Distributed Cognition, Formative Assessment, Self-Efficacy and Gamification, and Open Science and Procedural Justice across nine domains. The analysis is based on source code review (≈50,000 LOC, ≈290 components) and 30 design documents as primary sources, with seven months of anonymized activity logs consulted as supplementary reference (empirical validation of operational effects is deferred to follow-up tracks). Mapping was conducted across 90 cells (10×9) under a four-level strength rubric. The contributions are three-fold: a theory-implementation matrix as a central visualization, quantification of mapping strength via row/column means revealing theoretical integration density, and identification of six cases of theoretical synergy, tension, and complementarity. Results show that motivation-oriented theories exhibit the broadest distribution, while cognitive theories cluster around specific task domains such as the publication wizard. The matrix serves as a roadmap for six prioritized empirical follow-up studies.
...
본 논문의 학술적 기여는 세 갈래로 구체화할 수 있다. 매트릭스는 단순한 분류표가 아니다.

1. **이론-구현 매트릭스의 제시**: 10×9 매핑을 4단계 강도 기준(핵심·중요·부분·잠재)으로 평가하여 학술 가독성 높은 중심 도식으로 제공한다.
2. **매핑 강도의 정량화**: 행·열 평균을 통해 이론별 통합도와 도메인별 이론 밀도를 정량 비교한다.
3. **이론 통합 패턴 6개 사례 식별**: 시너지·긴장·보완 3유형 각각의 구체적 사례를 디자인 결정과 함께 분석한다.

결과는 학회·기관 차원의 학술 디지털 인프라 설계자에게 이론 기반 의사결정의 참조점이 되며, 매트릭스의 핵심(●●●) 셀은 후속 실증 연구의 6개 트랙으로 발전할 출발점이다. 분석 단위가 분명하다.

저자는 사이트의 운영진으로 활동하였으며, 이는 insider perspective의 분석적 장단점을 동반한다. 통제 절차는 부록 B(3원천 교차 검증)와 부록 E(증거 anchor 표)를 통해 본문에서 가시화되었다.
...
복수 이론의 통합 분석은 세 가지 도전을 안고 있다 — 이론 간 개념적 호환성 확보, 매핑 강도의 평가 기준 마련, 통합 시 발생하는 시너지·긴장의 식별이 그것이다(Bell, 2004). 이 도전에 대응하기 위해 본 연구는 framework synthesis(Carroll et al., 2013) 방법론을 채택하여, 사전 정의된 이론 framework를 분석 lens로 활용하면서 데이터에서 새로운 패턴이 드러나도록 하였다.

매핑 강도의 평가는 디자인 합리화 연구에서 흔히 reviewer의 주관성에 의존해 왔다. 이 연구는 4단계 rubric(§3.3)으로 평가 기준을 명시하고, 부록 B의 3원천 교차 검증(코드·설계 문서·인터뷰)과 부록 E의 ●●● 핵심 셀 증거 anchor 표를 통해 단일 평가자 구조의 한계를 통제하였다. 외부 평가자에 의한 독립 재평가는 §5.5의 후속 트랙 F2로 이전되었다.
...
**평가자 구조와 한계의 명시**: 본 매핑은 단일 평가자(저자) 기준으로 1차 도출되었다. 외부 평가자의 독립 재평가와 Cohen's κ 산출은 본 논문에서 수행되지 않았다. 평가의 주관성 잔존을 통제하기 위해 다음 절차가 적용되었다. 각 셀의 강도 등급은 부록 B의 3원천 교차 검증(코드·설계 문서·인터뷰 발화)에 기반하며, ●●● 핵심 셀과 ●● 중요 셀에 대한 증거 표는 부록 E(증거 anchors)로 분리·공개된다. 단일 평가자 구조의 한계는 §5.5의 L2 한계로 본문에 명시되었고, F2 후속 트랙(외부 평가자 풀 모집 + 비-운영진 독립 재평가)에서 후속 검증이 계획되어 있다. 평가의 재현 가능성은 부록 B의 검증 절차와 부록 E의 증거 표를 통해 외부 연구자가 본 매핑을 재현·반증할 수 있는 형태로 보장된다.
...
매트릭스는 단일 이론으로 환원되지 않는 학회 차원의 디지털 인프라가 다수의 교육공학 이론을 동시에 수용할 수 있음을 정량적·해석적으로 보여 주는 결과이다. 동기 군집(SDT·Self-Efficacy·SRL)이 가장 폭넓게 분포한다는 점은, 학회 회원의 활동을 관통하는 동기 차원의 이론들이 시스템 전반에서 작동한다는 관찰로 읽힌다. 인지 군집(CLT·CTML·Distributed Cognition)이 좁고 깊게 분포한다는 점은, 인지 차원이 특정 과제에서 집중적으로 작동한다는 해석을 뒷받침한다.
...
매트릭스 논문의 한계는 여섯 갈래로 나누어진다. 한 갈래의 한계가 어느 후속 연구로 이어지는지를 일대일로 짝지어 배치한다. 이 짝짓기는 한계 진술이 회의적 항복이 아니라 후속 연구의 청사진으로 기능하도록 의도된 구조이다. 짝짓기는 분명하다.

**(L1) 단일 사례 일반화의 한계.** 매트릭스는 한 학회의 특수 맥락에서 형성되었으며, 셀별 매핑 강도는 운영진의 개별적 이론 선호에 일부 의존한다. 이 한계와 연결하여, **(F1) CoP × 졸업생 DB 트랙**은 LPP 궤적을 따른 신규 회원의 학자 정체성 발달을 다른 학회 사례와 함께 종단적으로 검토하여 매트릭스의 적용 범위를 넓힌다.

**(L2) Insider perspective + 단일 평가자 구조의 한계.** 저자가 사이트 운영진으로 활동하였고, 본 매트릭스 단계에서 외부 평가자의 독립 재평가는 수행되지 않았다. 평가의 주관성 잔존은 부록 B의 3원천 교차 검증(코드·설계 문서·인터뷰)과 부록 E의 ●●●·●● 셀 증거 anchor 표를 통해 부분적으로 통제되었다. 이 한계와 연결하여, **(F2) 비-운영진 외부 평가자 풀에 의한 매트릭스 독립 재평가 트랙**은 박사과정·박사 후 연구원·시니어 faculty의 다층 경력 평가자 3~5인을 모집하여 본 매트릭스를 독립 재평가하고 Cohen's κ(가중·비가중) 또는 Krippendorff's α를 산출한다. 부록 D에 모집·평가 절차의 청사진이 정리되어 있다.

**(L3) 측정 지표의 미실증.** 매트릭스는 각 매핑에 대해 측정 가능 지표를 제안하였으나, 운영 효과의 실증은 논문의 범위 바깥에 둔다(§3.2, §4.4). 이 한계와 연결하여, **(F3) SRL × 학습 잔디 트랙**은 365일 streak이 자기조절학습 전략 사용에 미치는 영향을 MSLQ 종단 측정으로 검증한다.

**(L4) 시스템 진화 시 매트릭스 변동.** 사이트는 지속 진화하므로 매트릭스는 2026년 5월의 스냅샷이다. 이 한계와 연결하여, **(F4) Phase 2~4 시점 매트릭스 종단 재평가 트랙**은 사이트의 후속 Phase에서 동일한 4단계 rubric으로 매핑을 재산출하여 매트릭스의 안정성·이동성·새 셀의 출현을 추적한다. 매트릭스 논문은 이 종단 비교의 baseline(t=0) 역할을 한다.

**(L5) 매핑 강도 평가의 주관성 잔존.** 단일 평가자 구조에서 4단계 rubric의 적용은 평가자의 이론 친숙도에 의해 일부 흔들릴 수 있다. 이 한계와 연결하여, **(F5) Distributed Cognition × 공동 작성 트랙**은 optimistic locking 협업의 인지 부하 분담 패턴을 시스템 로그(diff 빈도·충돌 회복 시간) 기반의 행동 데이터로 측정한다. 후속 트랙들이 자기보고 척도에 의존할 위험을 행동 데이터로 보완하는 사례이며, 평가자 친숙도에 의존하지 않는 객관 측정 경로를 매트릭스의 한 셀에서 시연한다.

**(L6) 해석적 단일 방법론의 한계 — 양적-질적 삼각검증의 부재.** 매트릭스 분석은 framework synthesis 기반의 해석적 분석으로, 양적 검증과 질적 인터뷰의 삼각검증은 본 논문에 포함되지 않았다. 이 한계와 연결하여, **(F6) Cognitive Apprenticeship × 검수 트랙**은 severity 4단계 코멘트가 신진 저자의 학술 글쓰기 학습에 미치는 영향을 질적 인터뷰와 텍스트 변화량 분석의 혼합 설계로 다룬다. 양적-질적 삼각검증의 시연 사례이다.

여섯 짝의 한계-제언 매핑은 매트릭스 논문이 단일 논문에 그치지 않고 연구 프로그램으로 발전할 수 있는 통로를 제공한다.

---

## 6. 결론 (Conclusion)

매트릭스 논문은 한 대학원 교육공학 학회의 디지털 인프라가 10개의 교육공학 이론을 9개 도메인에서 동시에 구현하는 통합 사례를 분석한 결과이다. 결론은 세 개의 거시 명제로 정리한다. 각 명제는 결과 진술에서 출발하여 이론적 위치 부여를 거쳐 실천적 함의에 도달하는 구조로 전개된다.

### 6.1. 명제 1 — 동기 차원의 이론은 시스템 전반에 분산되고, 인지 차원의 이론은 특정 과제에 응집된다

**SDT·Self-Efficacy·SRL로 묶이는 동기 군집은 행 평균 1.67–2.44 범위에서 5–8개 도메인에 분포한 반면, CLT·CTML·Distributed Cognition으로 묶이는 인지 군집은 행 평균 0.78–1.11 범위에서 3–4개 도메인에 응집되었다.** 동기 군집의 도메인 분포 폭(5–8)이 인지 군집(3–4)보다 두 배 가까이 넓다는 사실 자체가 이 명제의 정량적 근거이다. 분포가 증거이다. 이 결과는 동기 이론이 학회 회원의 활동 전반을 관통하는 횡단적 성격을 갖고, 인지 이론은 특정 과제(예: 출판 마법사, 챕터 공동 작성)에서 정밀하게 작동한다는 이론적 위치 부여로 이어진다. Reeves(2006)가 지적한 단일 이론 환원의 한계는 이 횡단-종단 분포의 비대칭을 가시화함으로써 디자인 합리화의 단위가 어디에 놓여야 하는지를 결정한다. 실천적으로 학회·기관 차원 인프라 설계자는 동기 차원을 시스템 수준 정책(옵트인 기본값, 자율성 보호 메커니즘)으로, 인지 차원을 과제 단위 디자인(단계 분할, 신호화)으로 분리 적용할 근거를 확보한다.

### 6.2. 명제 2 — 이론 간 시너지·긴장·보완은 단일 디자인 결정 안에서 동시에 가시화될 수 있다

**6개의 통합 패턴(시너지 1, 긴장 2, 보완 3) 중 보완 사례 3건은 서로 다른 이론적 출발점이 동일한 디자인 결정(저자 동의 게이트, 학습 잔디, 졸업생 학위논문 DB)으로 수렴한다.** 이 결과는 디자인 합리화 연구의 분석 단위가 단일 이론에서 이론 간 상호작용으로 확장되어야 한다는 이론적 위치 부여로 이어진다. Bell(2004)이 제기한 이론 통합의 방법론적 도전은 사례에서 framework synthesis와 4단계 매핑 강도 rubric의 결합으로 부분적으로 응답되었다. 실천적으로 한 디자인 결정에 대한 다층 정당화(multi-layered justification)는 학회 운영진의 의사결정에 학술적 두께를 제공한다. 단일 이론 인용보다 복수 이론 수렴이 견고한 디자인 합리화가 된다는 점이 사례의 핵심 단서이다.

### 6.3. 명제 3 — 옵트인 메커니즘은 동기 이론 간 상호 긴장을 디자인 수준에서 해소하는 일반 원리로 작동한다

**Leaderboard·동료 활동 피드의 옵트인 기본값은 일반적으로 충돌하는 것으로 알려진 SDT(자율성)와 Gamification(PBL)을 디자인 결정 수준에서 양립시킨 사례이다.** Hanus와 Fox(2015)가 종단 연구에서 보고한 게임화의 내재 동기 침해 위험은 자율성 침해에서 발생한다. SDT의 자율성 욕구(Deci & Ryan, 2000)와 PBL의 외재 동기 자극은 서로 다른 동기 회로를 활성화하므로, 두 회로가 한 사용자 위에서 동시에 작동할 때 자율성 회로가 약해지면 PBL의 단기 효과가 내재 동기를 잠식하는 종단 결과로 이어진다. 옵트인 메커니즘은 자율성 회로의 사전 보존을 통해 두 회로의 동시 작동을 가능하게 한다. 이는 단순 기능 토글이 아니라 동기 이론 간 긴장을 디자인 수준에서 해소하는 일반 원리이다. 이론적 정당화의 회로는 다음과 같이 작동한다 — 자율성 보호가 PBL 효과의 종단 안정성을 결정하는 매개 변수가 되며, 옵트인은 이 매개를 디자인 수준에서 상수화한다. 실천적으로 학회·기관 인프라가 게이미피케이션 요소를 도입할 때 옵트인이 기본값이 되어야 한다는 설계 기준은 사례로부터 도출된다. 사례에서 옵트인 전환이 운영 2개월 시점의 회원 피드백에서 비롯되었다는 인터뷰 보고는, 옵트인 기본값이 사후 정책이 아니라 초기 디자인 단계에서 채택되어야 함을 추가로 뒷받침한다. 단일 사례에서 도출된 디자인 원리가 일반 원리로 격상되기 위해서는 비교 사례(다른 학회·기관 인프라)의 누적이 필요하며, 그 비교는 §5.5의 F1·F2 트랙에 의해 진행될 예정이다.

### 6.4. 이론 통합 디자인 — 새로운 분석 단위의 학술적 정의

세 명제는 단일 시스템에 다수 이론이 동시에 구현된다는 관찰을 매트릭스로 가시화한 결과이다. 매트릭스는 분석의 끝이 아니라 새로운 분석 단위의 출발점이다. 이 분석 단위를 "이론 통합 디자인(Theory-Integrated Design, TID)" 으로 명명한다. TID는 다음의 세 속성으로 정의된다.

먼저, TID는 *단일 이론으로 환원되지 않는 디자인 결정*을 분석 대상으로 삼는다. Reeves(2006)와 Bell(2004)이 지적한 통합의 도전을 한 디자인 결정이 동시 충족하는 이론의 집합을 분석함으로써 응답한다. 분석 단위가 바뀐다. 다음으로, TID는 *시너지·긴장·보완의 통합 패턴*을 디자인 결정의 학술적 두께로 본다. 한 디자인 결정에 대한 다층 정당화(multi-layered justification)는 단일 이론 인용보다 견고한 디자인 합리화로 작동한다. 마지막으로, TID는 *매트릭스 셀의 강도 분포*를 정량 비교의 기준으로 채택한다. 행·열 평균과 분포 폭은 이론 통합의 학회·기관 간 비교를 가능하게 하는 공통 척도이다.

매트릭스 논문은 단일 사례·insider perspective·측정 지표 미실증의 한계를 가지지만(§5.5의 L1–L6), §5.5에 명시된 6개의 한계-제언 짝(L1↔F1, L2↔F2, L3↔F3, L4↔F4, L5↔F5, L6↔F6)을 통해 단일 논문을 넘어 6편의 후속 실증 연구로 확장 가능한 연구 프로그램의 출발점을 제공한다. 6개 트랙의 우선순위는 다음 두 기준의 가중합으로 결정된다 — *측정 용이성*(데이터 가용성·도구의 표준성)과 *학술적 임팩트*(이론적 확장 가능성·실천 함의의 광범위성). 두 기준 모두에서 높은 점수를 받는 F3(SRL × 학습 잔디)와 F1(CoP × 졸업생 DB)이 1차 우선 트랙으로 권장된다. F2(독립 audit)는 학술 재현성 차원에서 평행 우선 트랙으로 진행되어야 한다.

학회 차원 디지털 인프라의 이론 통합 디자인이라는 분석 단위는 디자인 합리화 연구와 학회 운영 디지털화 연구의 교차점에 자리한다. 매트릭스는 두 연구 흐름을 연결하는 도구이며, TID는 그 연결을 학술 어휘로 명명하는 새로운 개념이다.

---

## 이해상충 선언 (Conflict of Interest Declaration)

저자는 분석 대상 사이트(yonsei-edtech)의 운영진으로 활동하였다. 이러한 insider perspective는 분석적 깊이를 제공하는 동시에 자기 정당화 편향의 위험을 동반한다. 단일 평가자 구조의 한계는 부록 B의 3원천 교차 검증(코드·설계 문서·인터뷰)과 부록 E의 ●●● 핵심 셀 증거 anchor 표로 통제되었고, §5.5 한계 절(L2)에 본문화되어 외부 평가자 풀의 독립 재평가(F2)가 후속 트랙으로 예고된다.
...
## 부록 E — ●●● 핵심 셀의 증거 anchor 표

본 부록은 매트릭스의 ●●● 핵심 매핑 셀에 대한 3원천 증거 anchor(코드 위치 / 설계 문서 / 인터뷰 발화)를 명시한다. 본 표는 단일 평가자 구조(§5.5 L2)의 보완 절차로, 외부 연구자가 매핑의 근거를 추적할 수 있도록 작성되었다. 매핑 강도 ●● 중요 셀에 대한 동일 형식의 anchor 표는 별도 첨부 자료로 분리한다(투고 시 supplementary).

| 셀 (이론 × 도메인) | 코드 anchor (파일·라인 또는 컴포넌트) | 설계 문서 anchor | 인터뷰 발화 anchor |
|------|----|----|----|
| SRL × 학습 잔디 | `src/lib/streak/streak-events.ts` (day-bucketed doc id + 활동별 가중치 정의) | `docs/01-plan/features/learning-streak.md` (3-phase 모형 명시) | 운영진 P1: "잔디 가중치는 학습자의 자기 모니터링이 핵심이라는 점에서 출발했다" |
| SDT × 분석 매트릭스 | `src/components/ContributionsMatrix.tsx` + `src/lib/leaderboard/feedOptIn.ts` | `docs/02-design/features/leaderboard-optin.md` (옵트인 기본값 명시) | 운영진 P2: "leaderboard를 자동 노출로 만들었으나, 운영 2개월 후 옵트인으로 전환" (§4.3.1) |
| CLT × 출판 마법사 | `src/app/collab/[teamId]/publish/[articleId]/page.tsx` (5단계 분할 라우팅) | `docs/02-design/features/publish-wizard-stages.md` (segmenting principle 명시) | 운영진 P3: "마법사 5단계는 한 번에 입력해야 하는 부담을 줄이려는 의도" |
| CoP × CoP 계보 | `src/app/alumni/thesis/` + `src/components/lineage/` (졸업생 학위논문 + 계보도) | `docs/02-design/features/alumni-thesis-db.md` (LPP 지원 명시) | 운영진 P1: "졸업생 학위논문이 신규 회원의 학회 정체성 입문 자료" |
| Cognitive Apprenticeship × 검수 게이트 | `src/lib/review/severity-types.ts` (4단계 severity 정의) | `docs/02-design/features/review-workflow.md` (scaffolding + coaching 결합 명시) | 운영진 P3: "검수 코멘트의 4단계는 신진 저자가 단계별로 학습할 수 있도록 설계" |
| Distributed Cognition × 공동 작성 | `src/lib/collab/optimistic-locking.ts` + `src/lib/chapter/assignedUserIds.ts` | `docs/02-design/features/chapter-collab.md` (인지 분담 + 동기화 명시) | 운영진 P2: "챕터별 권한 분담은 인지 분담의 명시화" |
| Formative Assessment × 검수 게이트 | `src/lib/review/state-machine.ts` (revision_requested ↔ submitted 전이) | `docs/02-design/features/review-state-machine.md` (iterative feedback 명시) | 운영진 P3: "revision 순환은 피드백 반복성의 직접 구현" |
| Open Science + Procedural J. × 저자 동의 | `src/lib/consent/author-consent-gate.ts` + `src/lib/credit/credit-roles.ts` | `docs/02-design/features/author-consent-gate.md` (voice + consistency + transparency 결합 명시) | 운영진 P1: "동의 게이트는 출판 전 분쟁을 사전 차단하는 절차적 정의 구현" |
| Open Science + Procedural J. × 데이터 공유 | `src/lib/article/dataLinks.ts` (외부 OSF/Zenodo 연계) | `docs/02-design/features/data-sharing-links.md` (data sharing 표준 명시) | 운영진 P1: "dataLinks는 학술 재현성을 위한 외부 저장소 연계" |

표 형식의 한정 — 본 표는 ●●● 핵심 매핑의 3원천 anchor를 *예시 수준*에서 보여 주며, 모든 anchor의 전수 anchor 표는 학회지 투고 시 supplementary material로 제공된다. 인터뷰 발화는 익명 코드(P1·P2·P3)로 표기되었고, 원 녹취는 부록 C의 윤리 처리에 따라 본 연구 종료 후 파기된다.

---

## 부록 D — 외부 평가자 풀 모집 계획 (후속 트랙 F2)

매트릭스 논문 단계에서는 외부 평가자에 의한 독립 재평가가 수행되지 않았다(§3.3 평가자 구조의 명시 절 참조). 본 부록은 §5.5의 한계 L2와 후속 트랙 F2에서 예정된 외부 평가자 모집·재평가 절차의 청사진을 정리한다.

**모집 대상 풀.** 교육공학 박사 학위 소지자 또는 박사과정 수료자 중, 본 사이트 운영 경험이 없는 외부 연구자 3~5인. 학습 분석·디자인 합리화·실천공동체·HCI 등 본 연구 주제와 인접한 세부 분야에서 다층 경력(박사과정 / 박사 후 연구원 / 시니어 faculty)을 균형 있게 포함한다.

**평가 도구 예정안.** 4단계 rubric을 명시한 평가 시트(`docs/papers/rater-sheet.xlsx` 양식)에 셀별 강도 코드와 1~2문장 근거를 기입한다. 평가자에게는 저자의 1차 매핑을 비공개 처리한 상태에서 독립 평가를 요청한다.

**신뢰도 지표 계획.** 평가자 간 합의도는 Cohen's κ(가중·비가중 양쪽 보고) 또는 Krippendorff's α(ordinal)로 계산하며, 사전-합의 분포 표와 불일치 셀에 대한 합의 회의 기록을 함께 보고한다.

**연결 한계.** 본 부록의 모집·재평가는 §5.5의 L2(단일 평가자 한계)에 대한 F2 후속 트랙의 핵심 절차이다. 후속 연구는 매트릭스 논문의 매핑 결과를 검증·반증·확장하는 독립 audit의 형식으로 진행될 예정이다.
