...
- insights Group D (선택): 기수별 로얄티 추이, 세그먼트 이동 추적 등 고급 기능 — 미착수
- `admin/*` ↔ `console/*` 이중 라우트: `admin/*`가 여전히 직접 접근 가능. 향후
  `/admin/*` → `/console/*` redirect 또는 소스를 `src/features/`로 이전하는 구조 개선 검토
