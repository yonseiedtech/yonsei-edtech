...
| `3f56af2c` | feat: 로얄티 스냅샷 cron 인프라 + 추이 그래프 |
...
4. 회원 보고서 "로얄티 추이" 섹션의 "지금 캡처" 버튼 → 첫 스냅샷 적재 후 차트 표시
   (cron 은 매주 월요일 자동 적재이므로, 즉시 검증하려면 수동 캡처 1회 필요)
...
- ~~insights Group D (선택) — 시계열 추이~~ → 완료: 기수별 평균 로얄티 + 활동 모멘텀 +
  로얄티 스냅샷 cron 인프라 + 추이 그래프. 스냅샷이 누적될수록 추이 분석 가치 증대.
- **초기 시딩 필요**: cron 은 매주 월요일 적재 → 즉시 추이를 보려면 회원 보고서
  "지금 캡처" 버튼으로 admin 이 수동 스냅샷 1~2회 적재 권장.
