...
export default async function ExternalPage() {
  ...
  try {
    const db = getAdminDb();
    const snap = await db.collection("activities").where("type", "==", "external").get();
    ...
    console.error("[activities/external] server prefetch failed", err);
