...
import { archiveVariablesApi, alumniThesesApi } from "@/lib/bkend";
import type { ArchiveVariable } from "@/types";
import { toast } from "sonner";

export default function ConsoleVariablesEditPage() {
  ...
  const [item, setItem] = useState<ArchiveVariable | null>(null);
  const [thesisIds, setThesisIds] = useState<string[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    if (user && !canManage) {
      router.replace("/archive/variable");
    }
    ...
    (async () => {
      ...
      try {
        const [loaded, thesesRes] = await Promise.all([
          archiveVariablesApi.get(params.id),
          alumniThesesApi.list(),
        ]);
        if (cancelled) return;
        setItem(loaded);
        const linked = thesesRes.data
          .filter((t) => (t.variableIds ?? []).includes(params.id))
          .map((t) => t.id);
        setThesisIds(linked);
      } catch (err) {
        console.error("[console-archive-variable-edit] load failed", err);
