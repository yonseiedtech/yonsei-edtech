...
export default function InviteDraftPicker({ value, onChange, excludeUserIds }: Props) {
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          {value.map((d, idx) => (
            <div
              ...
              className="flex items-center justify-between gap-2 rounded border border-zinc-200 bg-zinc-50 px-3 py-2"
            >
              <div className="flex-1">
                <p className="text-sm font-medium">{d.recipientName}</p>
                {d.recipientEmail && (
                  <p className="text-xs text-zinc-500">{d.recipientEmail}</p>
                )}
              </div>
              <select
      ...
      {/* 새 팀원 추가 */}
      <div className="space-y-3 rounded border border-dashed border-zinc-300 p-3">
