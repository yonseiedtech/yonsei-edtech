  ...
  return (
    <div>
      <div className="flex items-baseline justify-between">
        ...
        <p className="text-sm font-bold tabular-nums text-cat-5">
          {pct}
          <span className="ml-0.5 text-[10px] font-normal text-muted-foreground">/ 100</span>
        </p>
      </div>
      <div
        className="mt-1 h-1.5 overflow-hidden rounded-full bg-cat-5/10"
        role="progressbar"
        aria-valuenow={pct}
        aria-valuemin={0}
        aria-valuemax={100}
      ...
      >
        <div
          className="h-full rounded-full bg-cat-5 transition-all"
          style={{ width: `${pct}%` }}
        />
      </div>
    </div>
...
export default function DiagnosisReadinessWidget() {
  ...
  if (!latest) {
    return (
      <Link
        href="/diagnosis"
        className="flex items-center gap-3 rounded-2xl border-2 border-cat-5/30 bg-gradient-to-br from-cat-5/5 to-cat-5/10 p-4 transition-colors hover:border-cat-5/50"
      >
        <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-cat-5/15 text-cat-5">
        ...
        <ArrowRight size={16} className="shrink-0 text-cat-5" />
      </Link>
    );
  }

  return (
    <div className="rounded-2xl border-2 border-cat-5/30 bg-gradient-to-br from-cat-5/5 to-cat-5/10 p-4">
      <div className="flex items-center gap-2">
        <ClipboardCheck size={16} className="text-cat-5" />
      ...
      {Math.min(latest.paperReadiness, latest.analysisReadiness) < 100 && (
        <Link
          href="/diagnosis"
          className="mt-3 flex animate-in fade-in items-center gap-2 rounded-xl bg-cat-5/10 px-3 py-2 text-[11px] font-medium text-cat-5 transition-colors hover:bg-cat-5/15"
        >
      ...
      <Link
        href="/mypage"
        className="mt-3 inline-flex items-center gap-1 text-[11px] font-medium text-cat-5 hover:underline"
      >
        약점 개념 학습 경로 보기
        <ArrowRight size={11} />
      </Link>
