...
import { X, CalendarRange, Sparkles, BookOpen, UserCheck } from "lucide-react";
 ...
 * C-1: 실개강일 우선·관례일 폴백 (effectiveSemesterStart 재사용).
 */
function activeKickoffSemester(
  entries: { year: number; semester: "first" | "second"; semesterStart: string }[],
): { key: string; label: string } | null {
  const today = todayYmdKst();
  const [y] = today.split("-").map(Number);
  const candidates: { year: number; semester: "first" | "second"; key: string; label: string }[] = [
    { year: y, semester: "first", key: `${y}-1`, label: `${y}년 1학기` },
    { year: y, semester: "second", key: `${y}-2`, label: `${y}년 2학기` },
    { year: y + 1, semester: "first", key: `${y + 1}-1`, label: `${y + 1}년 1학기` },
  ];
  for (const c of candidates) {
    const startStr = effectiveSemesterStart(c.year, c.semester, entries);
...
export default function SemesterKickoffBanner() {
  ...
  const sem = activeKickoffSemester(calendarData.entries);
  ...
  // C-3: 휴학자 — 수강 등록 유도 대신 복학 안내·학사정보 최신화로 분기
  const isOnLeave = user.enrollmentStatus === "on_leave";
  ...
  if (isOnLeave) {
    return (
      <div className="relative mb-4 overflow-hidden rounded-2xl border-2 border-warning/30 bg-warning/5 p-4">
        ...
        <p className="text-sm font-bold">🎓 {sem.label} 개강이 다가오고 있어요</p>
        <p className="mt-0.5 text-xs text-muted-foreground">
          복학을 준비 중이라면 학사정보를 최신화해 주세요 — 학기 알림·대시보드가 복학 모드로 전환됩니다.
        </p>
        <div className="mt-2.5">
          <Link
            href="/mypage/academic-status"
            className="inline-flex items-center gap-1 rounded-lg border border-warning/40 bg-card px-2.5 py-1.5 text-xs font-medium text-warning-foreground transition-colors hover:bg-warning/10"
          >
            <UserCheck size={12} /> 학사정보 최신화
          </Link>
        </div>
      </div>
    );
