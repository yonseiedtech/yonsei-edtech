...
import type { InstrumentItem, ProcedureStep } from "@/types";
...
export function buildInstrumentTable(items: InstrumentItem[]): string {
...
/** 절차 프리셋 — 준실험(사전-처치-사후) 표준 골격 */
const PROCEDURE_PRESET: Omit<ProcedureStep, "id">[] = [
...
const INSTRUMENT_FIELDS: { key: keyof Omit<InstrumentItem, "id">; label: string; ph: string; w: string }[] = [
...
export default function MethodStructurePanel({
  ...
  const instTable = buildInstrumentTable(instruments);
            ...
            {!readOnly && (
              <div className="flex flex-wrap items-center justify-between gap-2">
                ...
                >
                  ...
                  &lsquo;측정 도구&rsquo; 절에 표 삽입
                </Button>
              </div>
            )}
            <p className="text-[10px] text-muted-foreground">
              본 연구 α는 분석 후 채워도 됩니다(빈칸은 ___). 신뢰도와 별개로 타당도(내용·구인) 확보 방법은 본문에 문장으로
              기술하세요.
            </p>
          </div>
        )}
          ...
          <div className="space-y-2 border-t border-violet-200/60 px-3.5 py-3 dark:border-violet-800/40">
            {procedure.length === 0 && !readOnly && (
              <div className="flex flex-wrap items-center gap-2 rounded-lg border border-dashed bg-card/50 px-3 py-2.5">
                <p className="text-[11px] text-muted-foreground">준실험 표준 골격으로 시작할 수 있어요:</p>
                <Button
                  size="sm"
                  variant="outline"
                  className="h-6 text-[11px]"
                  onClick={() =>
                    onProcedureChange(PROCEDURE_PRESET.map((s) => ({ ...s, id: newId("proc") })))
                  }
                >
                  사전 → 처치 → 사후 골격 넣기
                </Button>
            ...
            <p className="text-[10px] text-muted-foreground">
              처치 기간의 충분성(인지·태도 변화는 최소 5차시 내외)과 두 집단이 처치 외 조건에서 동일했음을 본문 문장으로
              보완하세요.
            </p>
          </div>
        )}
