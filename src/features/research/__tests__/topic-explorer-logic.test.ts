...
describe("topic-explorer 인터뷰 흐름", () => {
  ...
  it("현장이 없으면 개입(intervention)·규모(scale) 질문을 건너뛴다", () => {
    const a: TEAnswers = { field: "none", interest: "ai_edtech" };
  ...
  it("현상 이해형(understand)은 규모 질문을 건너뛴다", () => {
    const a: TEAnswers = { field: "school_k12", interest: "motivation", intervention: "survey_only", problem: "understand" };
...
describe("topic-explorer 추천", () => {
  it("효과 검증 + 개입 가능 → 양적(준실험) 프레임 우선", () => {
    const r = teRecommend({ field: "school_k12", interest: "ai_edtech", intervention: "can_intervene", problem: "effect", scale: "large" });
  ...
  it("효과 검증 + 소수 인원 → 주의 문구", () => {
    const r = teRecommend({ field: "school_k12", interest: "ai_edtech", intervention: "can_intervene", problem: "effect", scale: "small" });
    expect(r!.caution).toBeTruthy();
  });

  it("효과 검증 + 개입 불가(설문만) → 관계형 조사연구 + 인과 주의", () => {
    const r = teRecommend({ field: "corporate", interest: "hrd", intervention: "survey_only", problem: "effect", scale: "large" });
    expect(r!.frames[0].methodSeedKeys).toContain("research-method:survey");
    expect(r!.caution).toMatch(/관계/);
  });

  it("현상 이해형 → 질적 프레임", () => {
    const r = teRecommend({ field: "university", interest: "interaction", intervention: "survey_only", problem: "understand" });
    expect(r!.frames[0].approach).toBe("질적");
    expect(r!.frames.some((f) => f.approach === "혼합")).toBe(true);
  });

  it("개발형 + 개입 불가 → 델파이·척도 개발 경로", () => {
    const r = teRecommend({ field: "public", interest: "evaluation", intervention: "survey_only", problem: "build", scale: "unsure" });
  ...
  it("현장 없음 + 실태 파악 → 동향 분석(메타분석 경로)", () => {
    const r = teRecommend({ field: "none", interest: "ai_edtech", problem: "status" });
...
describe("topic-explorer 선배 논문·개념 매칭", () => {
  const result = teRecommend({ field: "school_k12", interest: "motivation", intervention: "can_intervene", problem: "effect", scale: "large" })!;
