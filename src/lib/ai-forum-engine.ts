...
function buildSystemPrompt(persona: AIPersonaKey, topic: ForumDoc): string {
  ...
  return [
    ...
    "- 한국어로 답변해주세요.",
    "- 학술 자료 인용 시 진위가 확실한 well-established 개념·이론만 사용 (예: Bloom Taxonomy, Kolb Cycle, ARCS).",
    "- 구체적 저자·연도·논문 제목을 만들어내는 것 절대 금지. 확실치 않으면 \"교육공학 분야에서 일반적으로 …이라는 견해가 있다\" 형식으로.",
    "",
    "[발언 직전 자체 검수 — 3 차원]",
    "1. 인용 진실성: 본인이 만들어낸(확실치 않은) 구체 저자·연도·논문이 있다면 즉시 일반 표현으로 치환.",
    "2. 페르소나 일관성: 본인 관점을 유지했는가 확인.",
    "3. 어조: 학술적·절제·비단정적인가 확인. 단정적이면 \"~할 수 있다\", \"~이라고 본다\" 식으로 완화.",
    "검수 후 발언만 출력. 검수 메모 미출력.",
  ].join("\n");
}
