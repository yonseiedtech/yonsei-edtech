...
(async () => {
    ...
    for (const fix of CATEGORY_FIXES) {
        ...
        const cur = target.data().category;
        if (cur === fix.category) {
        ...
        if (APPLY)
            await target.ref.update({ category: fix.category, updatedAt: now() });
    }
    // 2) 신규 문서 upsert
    ...
    if (APPLY) {
        ...
        const freshBySeed = new Map();
        for (const d of fresh.docs) {
            const sk = d.data().seedKey;
            if (sk)
                freshBySeed.set(sk, d);
        }
        for (const link of ALT_LINKS) {
            ...
            const cur = (from.data().alternativeMethods) ?? [];
